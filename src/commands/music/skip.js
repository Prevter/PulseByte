...
module.exports = class extends Command {
    ...
    async runAsSlash(interaction, locale, args) {
        ...
        if (!voiceChannel)
            return await interaction.reply({ embeds: [Command.createErrorEmbed(locale('skip.no_voice'))] });

        const queue = this.discord.distube.getQueue(interaction);
        if (!queue)
            return await interaction.reply({ embeds: [Command.createErrorEmbed(locale('skip.no_queue'))] });
    ...
    async run(message, locale, args) {
        ...
        if (!voiceChannel)
            return await message.reply({ embeds: [Command.createErrorEmbed(locale('skip.no_voice'))] });

        const queue = this.discord.distube.getQueue(message);
        if (!queue)
            return await message.reply({ embeds: [Command.createErrorEmbed(locale('skip.no_queue'))] });
